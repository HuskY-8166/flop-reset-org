...
export default async function History() {
  const [{ data: series }, { data: players }, { data: teams }] = await Promise.all([
    supabase.from('series').select('series_id, opponent_name, series_date, teams ( name, format ), matches ( flop_reset_score, opponent_score, is_forfeit )').order('series_date',{ascending:false}),
  ...
  for (const s of sorted as any[]) { const wins=s.matches?.filter((m:any)=>m.flop_reset_score>m.opponent_score).length??0; const losses=s.matches?.filter((m:any)=>m.flop_reset_score<m.opponent_score).length??0; if(wins>losses && !firstWins.has(s.teams?.name)) firstWins.set(s.teams?.name,s) }
  const nav = [
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
    <PageHero eyebrow="Permanent organization archive" title="Flop Reset History" description="The connected story of the teams, players, opponents, records, and results that built Flop Reset.">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4"><StatCard label="Archive Begins" value={archiveBegins ? dateLabel(archiveBegins) : '—'}/><StatCard label="Competitive Series" value={series?.length??0}/><StatCard label="Competitive Games" value={totalGames}/><StatCard label="Known Aliases" value={aliasCount}/></div>
    </PageHero>
    ...
    <section className="mt-16"><SectionHeader eyebrow="Milestones" title="First Recorded Victories" description="Derived only from dated competitive series in the archive."/>{firstWins.size ? <div className="grid gap-4 md:grid-cols-3">{[...firstWins.entries()].map(([team,s]:any) => {const wins=s.matches.filter((m:any)=>m.flop_reset_score>m.opponent_score).length,losses=s.matches.filter((m:any)=>m.flop_reset_score<m.opponent_score).length;return <Link key={team} href={`/matches/${s.series_id}`} className="rounded-2xl border border-neutral-800 bg-[#111] p-5 text-white no-underline hover:border-purple-800"><div className="text-xs font-bold uppercase tracking-wide text-purple-400">First recorded win</div><div className="mt-2 text-2xl font-black">{team}</div><div className="mt-1 text-sm text-neutral-500">vs {s.opponent_name} · {dateLabel(s.series_date)}</div><div className="mt-4"><ResultBadge wins={wins} losses={losses}/></div></Link>})}</div> : <EmptyState title="Milestones are waiting" description="They will appear when dated competitive results are available."/>}</section>
    <section className="mt-16"><SectionHeader eyebrow="Chronology" title="Recent Archive Entries" description="The latest completed series, preserved as the backbone of the organization timeline." href="/matches" linkLabel="Full match archive"/><div className="relative ml-3 border-l border-purple-900/60 pl-6">{(series??[]).slice(0,12).map((s:any)=>{const wins=s.matches?.filter((m:any)=>m.flop_reset_score>m.opponent_score).length??0,losses=s.matches?.filter((m:any)=>m.flop_reset_score<m.opponent_score).length??0;return <Link key={s.series_id} href={`/matches/${s.series_id}`} className="relative mb-4 block rounded-xl border border-neutral-800 bg-[#111] p-4 text-white no-underline hover:border-purple-800"><span className="absolute -left-[31px] top-5 h-3 w-3 rounded-full bg-purple-500"/><div className="text-xs text-neutral-600">{dateLabel(s.series_date)} · {s.teams?.format}</div><div className="mt-1 font-bold">{s.teams?.name} vs {s.opponent_name}</div><div className="mt-2"><ResultBadge wins={wins} losses={losses}/></div></Link>})}</div></section>
