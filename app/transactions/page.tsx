...
export default async function Transactions(){
  const {data:stats}=await supabase.from('match_player_stats').select('player_id, players ( name ), matches ( match_date, teams ( name, format ) )')
  const first=new Map<string,any>()
  ;(stats??[]).forEach((s:any)=>{const name=s.players?.name,date=s.matches?.match_date,team=s.matches?.teams;if(!name||!date)return;const key=`${name}|${team?.name}|${team?.format}`;if(!first.has(key)||date<first.get(key).date)first.set(key,{name,date,team:team?.name??'Unknown',format:team?.format??'Unknown'})})
  ...
  return <main className="mx-auto max-w-6xl px-4 py-10 md:px-8 md:py-14"><PageHero eyebrow="Roster archive foundation" title="Transactions" description="Verified roster changes will live here without overwriting historical membership. Until dated join/leave records exist, the page shows first recorded competitive appearances only."/>
    <section className="mt-12"><SectionHeader eyebrow="Evidence currently available" title="First Recorded Appearances" description="An appearance date is not presented as a signing date. It only proves the player competed under that squad by this date."/>{appearances.length?<div className="relative ml-3 border-l border-purple-900/60 pl-6">{appearances.map((a)=><article key={`${a.name}-${a.team}-${a.format}`} className="relative mb-4 rounded-xl border border-neutral-800 bg-[#111] p-4"><span className="absolute -left-[31px] top-5 h-3 w-3 rounded-full bg-purple-500"/><div className="text-xs text-neutral-600">{formatPublicDate(a.date)} · First recorded appearance</div><Link href={`/players/${encodeURIComponent(a.name)}`} className="mt-1 inline-block text-lg font-bold text-white hover:underline">{a.name}</Link><div className="text-sm text-neutral-500">{a.team} · {a.format}</div></article>)}</div>:<EmptyState title="No appearance history yet" description="This section will populate from dated competitive player statistics."/>}</section>
