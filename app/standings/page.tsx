...
export default async function Standings({ searchParams }: { searchParams: Promise<{ competition?: string }> }) {
  ...
  const [{ data: competitions }, { data: teams, error }, { data: series }] = await Promise.all([
    ...
    supabase.from('series').select('competition_id, flop_reset_team_id, notes, teams ( name, format ), matches ( * )'),
  ])
  ...
  const rows = selectedCompetition ? (teams ?? [])
    ...
    .map((team) => {
      ...
      for (const entry of validSeries.filter((row: any) => Number(row.flop_reset_team_id) === Number(team.id))) {
        const outcome = getSeriesOutcome((entry as any).matches ?? [], entry as any)
  ...
  return <main className="mx-auto max-w-6xl px-4 py-10 md:px-8 md:py-14">
    ...
    {error && <div className="rounded-xl border border-red-900 bg-red-950/20 p-4 text-red-300">Something went wrong while loading the standings. Please try again shortly.</div>}
    {withheld > 0 && <div className="mb-8 rounded-xl border border-amber-900/60 bg-amber-950/20 p-4 text-sm text-amber-200">Some historical results are temporarily unavailable while this table is being corrected.</div>}
