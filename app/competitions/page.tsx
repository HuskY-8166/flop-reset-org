...
export default async function Competitions() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
    ...
    <section className="mt-12">
      <SectionHeader eyebrow="Competition library" title="Recorded Circuits" description="Only series whose Flop Reset squad format matches the competition format count toward a hub." />
      {groups.size ? <div className="space-y-8">{[...groups.values()].map(({ identity, competitions: groupCompetitions }) => <section key={identity.groupKey} className="rounded-3xl border border-neutral-800 bg-[#0f0f0f] p-5 md:p-7">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">{groupCompetitions.sort((a, b) => String(a.format).localeCompare(String(b.format))).map((competition) => {
          ...
          const valid = attached.filter((row: any) => formatsMatch(competition.format, row.teams?.format))
          const withheld = attached.length - valid.length
          const upcoming = (scheduled ?? []).filter((row: any) => Number(row.competition_id) === Number(competition.id) && formatsMatch(competition.format, row.teams?.format)).length
          let wins = 0
          let losses = 0
          valid.forEach((row: any) => { const outcome = getSeriesOutcome(row.matches ?? []); if (outcome.won) wins += 1; else if (outcome.lost) losses += 1 })
          return <Link key={competition.id} href={`/competitions/${competition.id}`} className="group rounded-2xl border border-neutral-800 bg-[#151515] p-5 text-white no-underline hover:-translate-y-0.5 hover:border-purple-700">
            <div className="flex items-start justify-between gap-4"><div><div className="text-2xl font-black text-purple-300">{competition.format}</div><div className="mt-1 text-sm text-neutral-500">{valid.length} completed series · {upcoming} upcoming</div></div>{valid.length ? <ResultBadge wins={wins} losses={losses} /> : <span className="rounded-full border border-neutral-700 px-3 py-1 text-xs font-bold text-neutral-500">No valid results</span>}</div>
            {withheld > 0 && <div className="mt-4 rounded-lg border border-amber-900/50 bg-amber-950/20 px-3 py-2 text-xs text-amber-300">{withheld} mismatched series withheld pending data correction.</div>}
            <div className="mt-4 text-sm text-purple-300">Open {competition.format} hub →</div>
          </Link>
        })}</div>
      </section>)}</div> : <EmptyState title="No competitions recorded" description="Competition hubs will appear after competition metadata is created in Admin." />}
    </section>
  </main>
}
