...
export default async function Leaders({ searchParams }: { searchParams: Promise<{ format?: string }> }) {
  ...
  const { data: stats } = await supabase.from('match_player_stats').select('player_id, goals, assists, saves, shots, score, mvp, players ( name ), matches ( series_id, is_forfeit, teams ( name, format ) )')
  const formats=[...new Set((stats??[]).map((s:any)=>s.matches?.teams?.format).filter(Boolean))], selected=query.format&&formats.includes(query.format)?query.format:'All'
  const map=new Map<string,any>()
  ;(stats??[]).filter((s:any)=>!s.matches?.is_forfeit&&(selected==='All'||s.matches?.teams?.format===selected)).forEach((s:any)=>{const name=s.players?.name??'Unknown',format=s.matches?.teams?.format??'Unknown',team=s.matches?.teams?.name??'Unknown',key=`${name}|${team}|${format}`,r=map.get(key)??{name,team,format,games:0,series:new Set<number>(),goals:0,assists:0,saves:0,shots:0,score:0,mvps:0};r.games++;if(s.matches?.series_id!=null)r.series.add(s.matches.series_id);r.goals+=Number(s.goals??0);r.assists+=Number(s.assists??0);r.saves+=Number(s.saves??0);r.shots+=Number(s.shots??0);r.score+=Number(s.score??0);r.mvps+=s.mvp?1:0;map.set(key,r)})
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14"><PageHero eyebrow="Career archive" title="All-Time Leaders" description="Competitive career totals across every recorded Flop Reset player identity, kept separate by format."/>
    ...
    {!rows.length?<EmptyState title="No leaders yet" description={`No ${selected==='All'?'':`${selected} `}competitive player statistics are available.`}/>:<section><SectionHeader eyebrow="Competitive totals" title={`${selected} Career Leaderboards`} description="Forfeits are excluded. Historical teams come from each match, and tied values share a competition rank."/><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{categories.map(([label,key])=>{const sorted=[...rows].filter((r)=>r[key]>0).sort((a,b)=>b[key]-a[key]);const leaders=competitionRanks(sorted,(r)=>String(r[key])).filter((entry)=>entry.rank<=5);return <article key={key} className="rounded-2xl border border-neutral-800 bg-[#111] p-5"><div className="text-xs font-bold uppercase tracking-wide text-neutral-500">{label}</div><div className="mt-4 space-y-3">{leaders.map(({row:r,rank})=><div key={`${r.name}-${r.team}-${r.format}`} className="flex items-start gap-3 border-t border-neutral-800 pt-3 first:border-0 first:pt-0"><span className="w-5 font-mono text-neutral-600">{rank}</span><div className="min-w-0 flex-1"><Link href={`/players/${encodeURIComponent(r.name)}`} className="font-bold text-white hover:underline">{r.name}</Link><div className="text-xs text-neutral-600">{r.team} · {r.format}</div></div><span className="font-mono font-bold text-purple-300">{r[key].toLocaleString()}</span></div>)}</div></article>})}</div></section>}
  </main>
}
