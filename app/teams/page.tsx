...
export default async function Teams() {
  ...
  const teams3v3 = teams?.filter((t) => t.format === '3v3' && t.active !== false) ?? []
  const teams2v2 = teams?.filter((t) => t.format === '2v2' && t.active !== false) ?? []
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        ...
        {list.map((team) => {
          ...
          <div
            key={team.id}
            id={team.name}
            style={{ borderTopColor: brand.secondaryColor }}
